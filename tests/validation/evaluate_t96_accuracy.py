...
def generate_accuracy_plots(spatial_results):
    """Generate plots showing spatial distribution of errors."""
    print("\n" + "="*80)
    print("GENERATING ACCURACY PLOTS")
    print("="*80)
    
    with PdfPages('t96_accuracy_report.pdf') as pdf:
        # Plot 1: Error vs distance for each region
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        axes = axes.flatten()
        
        for idx, (region_name, data) in enumerate(spatial_results.items()):
            if idx < len(axes):
                ax = axes[idx]
                r = np.sqrt(data['x']**2 + data['y']**2 + data['z']**2)
                
                # Scatter plot
                scatter = ax.scatter(r, data['rel_err']*100, 
                                   c=data['magnitude'], s=10, alpha=0.6,
                                   cmap='viridis')
                
                ax.set_xlabel('Distance (Re)')
        ...
        if len(spatial_results) < len(axes):
            fig.delaxes(axes[-1])
        ...
        for region_name, data in spatial_results.items():
            # Plot histogram
            ax.hist(data['rel_err']*100, bins=50, alpha=0.5, label=region_name,
    ...
    print("Plots saved to t96_accuracy_report.pdf")
...
def main():
    ...
    # Generate plots
    generate_accuracy_plots(spatial_results)
