...
FIXED VERSION V2: 
- Boundary check happens BEFORE the step (matching scalar)
- Limits step size near boundaries to avoid overshooting
"""
...
def adjust_step_sizes(r, r0, dir, ds_array, active_mask, rlim=None):
    ...
    # BUT limit step size near outer boundary to avoid overshooting
    if rlim is not None:
        mask_near_outer = (r >= 5) & (r > rlim - 2.0) & active_mask
        if np.any(mask_near_outer):
            # Limit step size to 0.5 Re near boundary
            ds_array[mask_near_outer] = np.sign(ds_array[mask_near_outer]) * np.minimum(
                np.abs(ds_array[mask_near_outer]), 0.5
            )


def trace_vectorized_no_interp_fixed_v2(xi, yi, zi, dir=1, rlim=10, r0=1, parmod=2,
    ...
    for step in range(1, maxloop):
        ...
        # Check outer boundary conditions BEFORE step (matching scalar)
        mask_outer = ((r >= rlim) | (ryz >= 1600) | (x >= 20)) & active_mask
        if np.any(mask_outer):
            status[mask_outer] = 1
            active_mask[mask_outer] = False
            # NO INTERPOLATION - stop at current position (matching scalar)
        
        # Only take step for traces that are still active
        if not np.any(active_mask):
            break
        
        # Store previous radial distances for active traces
        rr[active_mask] = r[active_mask]
        
        # Adjust step sizes based on radial distance (with boundary limit)
        ...
        mask_inner_cross = (r_new < r0) & (rr > r_new) & active_mask
        if np.any(mask_inner_cross):
            # Interpolate to exact boundary crossing (scalar does this)
            r1 = (r0 - r_new[mask_inner_cross]) / (rr[mask_inner_cross] - r_new[mask_inner_cross])
